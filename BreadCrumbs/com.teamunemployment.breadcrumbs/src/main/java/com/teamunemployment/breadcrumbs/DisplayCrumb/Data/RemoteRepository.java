package com.teamunemployment.breadcrumbs.DisplayCrumb.Data;

import com.teamunemployment.breadcrumbs.DisplayCrumb.Data.CrumbRepositoryContract;

/**
 * Created by jek40 on 30/06/2016.
 */
public class RemoteRepository implements CrumbRepositoryContract {
    @Override
    public String LoadCrumbDescription(String crumbId) {
        return null;
    }

    @Override
    public String LoadCrumbPlaceName(String id) {
        return null;
    }
}
