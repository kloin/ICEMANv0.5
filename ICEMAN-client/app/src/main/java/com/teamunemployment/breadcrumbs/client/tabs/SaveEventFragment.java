...
//        actionBar.hide();
	...
	private void createNewEvent() {
		...
		/*		for (PlaceLikelihood placeLikelihood : likelyPlaces) {
				...
				eventId +=1;
				PreferenceManager.getDefaultSharedPreferences(context).edit().putInt("EVENTID", eventId).commit();
				String eventIdString = Integer.toString(eventId);
				dbc.SaveCrumb(TrailId, eventIdString, lat, longit, ".jpg", timeStamp, getBitmapAsByteArray(media));
