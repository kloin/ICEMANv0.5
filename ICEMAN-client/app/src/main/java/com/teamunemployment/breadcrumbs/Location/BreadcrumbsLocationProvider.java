...
public class BreadcrumbsLocationProvider implements LocationListener {
            ...
            if (activity != null) {
                ...
                switch (msg.what) {
                    case MESSAGE_ON_GEOFENCE_EVENT: {
                        ...
                        // Fetch event Id
                        String eventId = PreferenceManager.getDefaultSharedPreferences(mActivity.mContext).getString("EVENTID", null);
                        DateTime timeStamp = new DateTime();
                        if (eventId == null) {
                            throw new NullPointerException("Event Id was null while trying to save metadata to database");
                        }
                        // Geofence is broken - start up gps again
                        activity.ListenPassivelyForGPSUpdatesInBackground(0);
                        // Save break point to metadata
                        mActivity.dbc.AddMetadata(eventId, timeStamp.toString(), location.getLatitude(), location.getLongitude(), "");
                    }
                }
            }
        }
    ...
    public void AddGeofences(Location location) {
        mApi.removeGeofences();
        mApi.addGeofence("MYGEOFENCE2", location.getLatitude(), location.getLongitude(), 200, BreadCrumbsPathSenseGeofenceEventReceiver.class);
    }
    ...
    private void StartListening(LocationListener listener) {
        ...
        androidLocationManager.requestLocationUpdates(androidLocationManager.GPS_PROVIDER, 900000, 500, listener);
    }
    ...
    private LocationListener locationListener = new LocationListener() {
        @Override
        public void onLocationChanged(Location location) {
            Toast.makeText(mContext, "shit happens", Toast.LENGTH_LONG).show();
